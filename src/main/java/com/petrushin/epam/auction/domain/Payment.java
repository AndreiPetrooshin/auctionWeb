package com.petrushin.epam.auction.domain;
...
public class Payment implements Identified {
    ...
    public static final String UPDATE_PAYMENT = "UPDATE payments SET fl_id=?, user_id=?, price=?, is_paid=? " +
            "WHERE payment_id=?";
    public static final String GET_BY_USER_ID = "SELECT p.payment_id, p.fl_id, p.user_id, p.price, p.is_paid," +
            "f.fl_type, f.fl_name, f.fl_description, f.fl_start_price, f.fl_state," +
            "u.u_login, u.u_password, u.u_email, r.role_id, r.user_role FROM payments p, flower_lot f, user u, user_role r " +
            "WHERE p.fl_id=f.fl_id AND p.user_id=u.user_id AND u.role_id=r.role_id AND p.user_id=?";

    public static final String GET_BY_LOT_ID = "SELECT p.payment_id, p.fl_id, p.user_id, p.price, p.is_paid," +
            "f.fl_type, f.fl_name, f.fl_description, f.fl_start_price, f.fl_state," +
            "u.u_login, u.u_password, u.u_email, r.role_id, r.user_role FROM payments p, flower_lot f, user u, user_role r " +
            "WHERE p.fl_id=f.fl_id AND p.user_id=u.user_id AND u.role_id=r.role_id AND p.fl_id=?";
