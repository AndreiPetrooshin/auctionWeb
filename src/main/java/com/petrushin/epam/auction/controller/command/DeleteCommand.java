    ...
    private String deleteAddress(HttpServletRequest request) {
        ...
        if (!idValue.isEmpty()) {
            ...
            try {
                ...
                session.removeAttribute(ATTR_DELETE_ADDR_ERROR);
            } catch (EntityDAOException e) {
                LOGGER.error("Delete address operation error", e);
                session.setAttribute(ATTR_DELETE_ADDR_ERROR, true);
            }
        }
    ...
    private String deleteCard(HttpServletRequest request) {
        ...
        if (!idValue.isEmpty()) {
            ...
            try {
                ...
                session.removeAttribute(ATTR_DELETE_CARD_ERROR);
            } catch (EntityDAOException e) {
                LOGGER.error("Delete card operation error", e);
                session.setAttribute(ATTR_DELETE_CARD_ERROR, true);
            }
        }
    ...
    private String deleteLot(HttpServletRequest request) {
        ...
        if (!idValue.isEmpty()) {
            ...
            try {
                ...
                session.removeAttribute(ATTR_DELETE_LOT_ERROR);
            } catch (EntityDAOException e) {
                LOGGER.error("Delete lot operation error", e);
                session.setAttribute(ATTR_DELETE_LOT_ERROR, true);
            }
        }
        return Pages.PROFILE_ABOUT_MYSELF_PAGE;
    }
